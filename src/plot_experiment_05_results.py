    ...
    for row, tt in enumerate(transition_times):
        ...
        for algo in algorithms:
            ...
            _plot_mean_std(
                ...
                run_labels=run_labels,
                report_context=f"TD-MSE | tt={tt}, algo={algo}",
            )
            _plot_mean_std(
                ax_val,
                ...
                n_remove_highest_mean_curves=n_remove_highest_mean_curves,
                run_labels=run_labels,
                report_context=f"D_rel | tt={tt}, algo={algo}",
            )
    ...
    if final_response_curves:
        ...
        for algo in sorted(by_algo.keys()):
            _plot_response_mean_std(
                ...
                n_remove_highest_mean_curves=n_remove_highest_mean_curves,
                run_labels=run_labels,
                report_context=f"Final-response | algo={algo}",
            )
    else:
...
def main() -> None:
    ...
    n_remove_highest_mean_curves = 2  # Set >0 to remove top-N highest-mean runs per plotted group.
    plot_results(experiment_name, n_remove_highest_mean_curves=n_remove_highest_mean_curves)
